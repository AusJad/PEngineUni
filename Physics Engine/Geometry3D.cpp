...
physvec3 GetMin(const AABB& aabb) {
...
physvec3 GetMax(const AABB& aabb) {
	physvec3 p1 = aabb.position + aabb.size;
	physvec3 p2 = aabb.position - aabb.size;
...
AABB FromMinMax(const physvec3& min, const physvec3& max) {
...
bool OverlapOnAxis(const AABB& aabb, const OBB& obb, const physvec3& axis) {
...
bool OverlapOnAxis(const AABB& aabb, const Triangle& triangle, const physvec3& axis) {
	Interval a = GetInterval(aabb, axis);
...
Interval GetInterval(const AABB& aabb, const physvec3& axis) {
...
void SplitBVHNode(BVHNode* node, const Mesh& model, int depth) {
	...
	if (node->children == 0) {
		...
		if (node->numTriangles > 0) {
			...
			node->children[0].bounds = AABB(c + physvec3(-e.x, +e.y, -e.z), e);
			node->children[1].bounds = AABB(c + physvec3(+e.x, +e.y, -e.z), e);
			node->children[2].bounds = AABB(c + physvec3(-e.x, +e.y, +e.z), e);
			node->children[3].bounds = AABB(c + physvec3(+e.x, +e.y, +e.z), e);
			node->children[4].bounds = AABB(c + physvec3(-e.x, -e.y, -e.z), e);
			node->children[5].bounds = AABB(c + physvec3(+e.x, -e.y, -e.z), e);
			node->children[6].bounds = AABB(c + physvec3(-e.x, -e.y, +e.z), e);
			node->children[7].bounds = AABB(c + physvec3(+e.x, -e.y, +e.z), e);
...
physmat4 GetWorldMatrix(const Model& model) {
...
physvec3 Unproject(const physvec3& viewportPoint, const vec2& viewportOrigin, const vec2& viewportSize, const physmat4& view, const physmat4& projection) {
...
Ray GetPickRay(const vec2& viewportPoint, const vec2& viewportOrigin, const vec2& viewportSize, const physmat4& view, const physmat4& projection) {
